...
static inline uint32_t midi_status(MIDI_t *restrict midi)
{
  return READ_REG(midi->SR);
}

static inline bool midi_rx_not_empty(MIDI_t *restrict midi)
{
  return READ_BIT(midi->SR, MIDI_SR_RXNE) != 0U;
}

static inline uint8_t midi_rx_level(MIDI_t *restrict midi)
{
  return (uint8_t)_FLD2VAL(MIDI_SR_LEVEL, READ_REG(midi->SR));
}
...
static inline void midi_clear_status(MIDI_t *restrict midi, uint32_t mask)
{
  WRITE_REG(midi->ICR, mask & (MIDI_ICR_OVF | MIDI_ICR_FERR));
}
